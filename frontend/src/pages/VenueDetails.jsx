...
const BookingModal = ({
  ...
  const handleDateChange = (e) => {
    ...
    if (courts[selectedCourtIdx]) {
      checkAvailability(
        newDate,
        courts[selectedCourtIdx]?.name || `Court ${selectedCourtIdx + 1}`
      );
    }
  };
  ...
  const handleCourtChange = (e) => {
    ...
    if (selectedDate && courts[newCourtIdx]) {
      checkAvailability(
        selectedDate,
        courts[newCourtIdx]?.name || `Court ${newCourtIdx + 1}`
      );
    }
  };
              ...
              >
                {courts.map((c, idx) => (
                  <option key={idx} value={idx}>
                    {c?.name || `Court ${idx + 1}`} — ₹
                    {Number(c?.perHourPrice ?? c?.pricePerHour ?? 0)} / hour
                  </option>
                ))}
              </select>
            </div>
...
const VenueDetails = () => {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        const { data } = await axios.get(`${base}/venues/${id}/ratings`);
        // Backend returns the venue with populated ratings
        const ratingsData = Array.isArray(data)
          ? data
        ...
        if (user) {
          const mine = ratingsData.find(
            (r) => (r.user?._id || r.user?.id) === user._id
          );
  ...
  const handleSubmitRating = async (e) => {
    ...
    try {
      ...
      if (myRating) {
        await axios.put(
          `${base}/ratings/${myRating._id || myRating.id}`,
        ...
        await axios.post(
          `${base}/ratings/`,
          { venueId: id, score: ratingScore, comment: ratingComment },
          { withCredentials: true }
        );
      }
      // refresh venue and ratings
      const [venueRes, ratingsRes] = await Promise.all([
        axios.get(`${base}/venues/${id}`),
        axios.get(`${base}/venues/${id}/ratings`),
      ]);
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-6">
          ...
          {user &&
            ...
            venue.owner === user._id && (
              <button
                onClick={() =>
                  navigate(`/venues/${venue._id || venue.id}/edit`)
                }
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <span>
                    ...
                    {venue?.totalRatings || ratings.length})
                  </span>
                </div>
              </div>

              {user && (
                <form onSubmit={handleSubmitRating} className="mb-6 space-y-3">
                ...
                {ratings.map((r) => (
                  <div key={r._id || r.id} className="border rounded-lg p-4">
                    ...
                    <div className="text-yellow-500 text-sm mb-1">
                      {"★".repeat(Math.max(0, Math.min(5, r.score || 0)))}
                      {"☆".repeat(Math.max(0, 5 - (r.score || 0)))}
                    </div>
                    {r.comment && (
                      ...
                      <table className="min-w-full">
                        <thead>
                          <tr className="border-b">
                            <th className="text-left py-3 pr-4 font-medium text-gray-900">
                              Court
                            </th>
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {courts
                            .filter((c) => {
                              ...
                              return (
                                courtName.includes(sportName) ||
                                sportName.includes(courtName)
                              );
                            })
                            .map((c, idx) => {
                              const price =
